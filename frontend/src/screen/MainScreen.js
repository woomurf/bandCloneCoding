...
class MainScreen extends Component {
  ...
  async loadGroupInfo() {
    await axios.get(`/group/${this.state.groupId}`).then(
      function (res) {
        this.setState({
          group: res.data,
        });
  ...
  async memberSelectEvent(searchParam) {
    if (searchParam !== "") {
      await axios.get(`/user/search/${searchParam}`).then(
        function (res) {
      ...
      await axios.get("/user/list").then(
        function (res) {
          this.setState({
  ...
  async updatePicturetList() {
    const pictureCollect = await this.getPicture();
    this.setState({ pictureCollect: pictureCollect });
  }

  async componentDidMount() {
    this.updatePicturetList();
      ...
      <div>
        ...
        <div>
          <div id="pageBody">
            ...
            <MainRside
              //DB 연결이되면 수정
              pictureCollect={this.state.pictureCollect}
            />
          </div>
        </div>
        <GroupInfoPopup
        ...
        <AlertPopup
          ...
          alertPopupOnoff={function () {
            ...
            if (
              ...
              this.state.confirmEvent === "Withdrawal"
            ) {
              this.props.onClick("");
            }
          }.bind(this)}
      ...
      case "post":
        tabPage = (
          <PostFrame
            ...
            updatePicturetList={this.updatePicturetList.bind(this)}
          />
        );
        break;
      case "member":
      ...
      case "setting":
        tabPage = (
          <SettingFrame
            ...
            profileImage={
              this.state.members[this.state.myIndex].profileImageUrl
            }
